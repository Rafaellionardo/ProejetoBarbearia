...
public class moedaFormatada extends JTextField {
    ...
    public moedaFormatada(int casasDecimais) {
        this(new DecimalFormat((casasDecimais == 0 ? "#,##0" : "#,##0.") + makeZeros(casasDecimais)));
    }

    public moedaFormatada() {
        this(MONETARY_FORMAT);
    }

    public moedaFormatada(NumberFormat format) {// define o formato do
        // número
        ...
        setDocument(new PlainDocument() {
            ...
            public void insertString(int offs, String str, AttributeSet a) throws BadLocationException {
                String text = new StringBuilder(moedaFormatada.this.getText().replaceAll("[^0-9]", "")).append(str.replaceAll("[^0-9]", "")).toString();
